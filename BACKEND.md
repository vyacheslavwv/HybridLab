...
Документ подготовлен для защиты проекта на конкурсе. Описывает **актуальную** реализацию в каталоге `metrics-backend/` — REST API на **FastAPI** для приёма и хранения телеметрии симулятора в **Excel**.
...
**Почему Excel, а не PostgreSQL?**  
Учебный контекст: преподаватель и студент открывают файл без СУБД; наглядность на конференции; простой экспорт. Для масштабирования можно заменить слой хранения, API останется тем же.

**Почему FastAPI?**  
Автоматическая валидация, OpenAPI из коробки, асинхронность, минимальный код для REST.

**Как защищены данные?**  
Валидация диапазонов на входе; HTTPS на периметре; изоляция в контейнере; логи на volume сервера.

**Что если придут неверные данные?**  
Pydantic вернёт 422 с указанием поля — в Excel ничего не попадёт.

**Как бэкенд масштабируется?**  
Текущая версия — single-instance logger. Горизонтальное масштабирование потребует общее хранилище (S3, БД) вместо локального xlsx.

**Зачем `/health`?**  
Оркестраторы и мониторинг проверяют, что процесс жив; стандартная практика для Docker/Kubernetes.
...
curl -X POST http://localhost:8000/metrics -H "Content-Type: application/json" -d "{\"pid_0d\":50,\"pid_0c\":2000,\"pid_5c\":90,\"hv_soc\":80,\"hv_volt\":300,\"electric_motor_torque\":100}"
